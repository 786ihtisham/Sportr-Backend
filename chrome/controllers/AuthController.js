	...
	(req, res) => {
		try {
			...
			if (!errors.isEmpty()) {
				...
				var addNewSourceModal = new AddNewSourceModal(
					{
						url: req.body.url,
						favicon: req.body.favicon,
						userId: req.body.userId,
				...
				AddNewSourceModal.save(function (err) {
					if (err) { return apiResponse.ErrorResponse(res, err); }
					return apiResponse.successResponseWithData(res,"Sources saved Successfully.", addNewSourceModal);
				});
			}
		} catch (err) {
			return apiResponse.ErrorResponse(res, err);
